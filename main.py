...
voting_clf_soft = load_model("voting_clf.pkl")
voting_clf_hard = load_model("voting_clf-hard.pkl")
xgboost_feature_engineered = load_model("xgboost-featureEngineered.pkl")
xgboost_smote = load_model("xgboost-SMOTE.pkl")
...
def make_predictions(input_df, input_dict):
  ...
  with col2:
      ...
      st.plotly_chart(fig_probs, user_container_width=True)
...
def explain_prediction(probability, input_dict, surname):
  ...
  Here are some statistics for churned customers:
  {df[df['Exited'] == 1].describe()}

  - If the customer has over a 40% risk of churning, generate 3 sentence explaination of why they are at risk of churning.
  - If the customer has less than 40% risk of churning, generate 3 sentence explanation of why they might not be at risk of churning.
  ...
  Don't mention the probability of churning, or the machine learning model, or say anything about like "Based on the machine learning model's prediction and top 10 most important features", just explain the prediction.
...
def generate_email(probability, input_dict, explanation, surname):
  prompt = f"""You are a manager at HS Bank. You are responsible for ensuring customers stay with the bank and are incentivized with various offers. Your name is Yohannes.

  You noticed a cusotmer name {surname} has a {round(probability * 100, 1)}% probability of churning.
...
if selected_customer_option:
  ...
  print("Selected Custom ID", selected_customer_id)

  selected_customer_name = selected_customer_option.split(" - ")[1]
  print("Selected Customer Name", selected_customer_name)
